...
/* 这里的子程序是通过括号中的变量传递参数,
   如果参数较多,为了节约RAM,也可以参考CH375子程序库改成通过同一全局变量/联合结构CH376_CMD_DATA传递 */

/* name 参数是指短文件名, 可以包括根目录符, 但不含有路径分隔符, 总长度不超过1+8+1+3+1字节 */
/* PathName 参数是指全路径的短文件名, 包括根目录符、多级子目录及路径分隔符、文件名/目录名 */
/* LongName 参数是指长文件名, 以UNICODE小端顺序编码,以两个0字节结束, 使用长文件名子程序必须先定义
   全局缓冲区GlobalBuf, 长度不小于64字节, 可以与其它子程序共用 */

/* 定义 NO_DEFAULT_CH376_INT 用于禁止默认的Wait376Interrupt子程序,禁止后,应用程序必须自行定义一个同名子程序 */
/* 定义 DEF_INT_TIMEOUT 用于设置默认的Wait376Interrupt子程序中的等待中断的超时时间/循环计数值, 
   0则不检查超时而一直等待 */
...
/* 定义 EN_LONG_NAME 用于提供支持长文件名的子程序默认是不提供 */

#include "header.h"
#include "h_macro.h"

BYTE    buf[64];
#define GlobalBuf    buf    
...
UINT8 CH376SendCmdWaitInt( UINT8 mCmd )  /* 发出命令码后,
等待中断 */
{
    xWriteCH376Cmd( mCmd );
    xEndCH376Cmd( );
...
UINT8 CH376SendCmdDatWaitInt( UINT8 mCmd, UINT8 mDat )  /* 发出
命令码和一字节数据后,等待中断 */
{
    xWriteCH376Cmd( mCmd );
...
/* StopName 指向最后一级文件名或者目录名 */
{
    ...
    while ( 1 ) {
        ...
        {  /* 路径尚未结束 */
            if ( s != ERR_OPEN_DIR ) 
            {  /* 因为是逐级打开,尚未到路径结束,所以,如果不是成功打开了目录,那么说明有问题 */
...
UINT8 CH376FileErase( PUINT8 PathName )  
{
    ...
    {   /* 文件尚未打开 */
        /* 搜索下一个路径分隔符或者路径结束符 */
        for ( s = 1; PathName[s] != DEF_SEPAR_CHAR1 && 
            PathName[s] != DEF_SEPAR_CHAR2 && PathName[s] != 0; ++ s ); 
        if ( PathName[s] ) 
...
/* 以字节为单位从当前位置读取数据块 */
UINT8 CH376ByteRead( PUINT8 buf, UINT16 ReqCount, UINT *RealCount ) 
{
    UINT8 s;
...
/* 以字节为单位向当前位置写入数据块 */
UINT8 CH376ByteWrite(PUINT8 buf,UINT16 ReqCount,UINT *RealCount ) 
{
    UINT8 s;
...
UINT8 CH376DiskCapacity( PUINT32 DiskCap )  /* 查询磁盘物理容量,扇区数 */
{
    ...
    if ( s == USB_INT_SUCCESS ) {  /* 参考Tf_Ch376Inc.h文件中CH376_CMD_DATA结构的DiskCapacity */
        xWriteCH376Cmd( CMD01_RD_USB_DATA0 );
...
UINT8 CH376DiskQuery( PUINT32 DiskFre )  /* 查询磁盘剩余空间信息,扇区数 */
{
    ...
    if ( s == USB_INT_SUCCESS ) {  /* 参考Tf_Ch376Inc.h文件中CH376_CMD_DATA结构的DiskQuery */
        xWriteCH376Cmd( CMD01_RD_USB_DATA0 );
...
/* 使用了全局缓冲区GlobalBuf的前12个字节 */
{
    ...
    else s = CH376FileOpen( (UINT8*)"/" );  /* 根目录下的文件,则打开根目录 */
...

{
    ...
    UINT  count;            /* 临时变量,用于计数,用于字节读文件方式下实际读取的字节数 */
    UINT  NameCount;        /* 长文件名字节计数 */
    UINT32 NewFileLoc;        /* 当前文件目录信息在上级目录中的起始位置,偏移地址 */

    for ( count = 0; count < LONG_NAME_BUF_LEN; count += 2 )
        if ( *(PUINT16)(&LongName[count]) == 0 ) break;  /* 到结束位置 */
