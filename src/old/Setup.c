...
BYTE numItem;	//iPort拥有的菜单页数
	...
	lcd_init(); 
	ports=C0S;		//GetByte(M_CFG,REG0+MAIN_NUM_OF_PORTS);
	if(ports>9){
...
UINT idle_cmp;
	...
	*fItemSaved=FALSE;
  	if(ReadPortSetup(iPort,iItem,numItem,&screen[0])==OK){
		ShowPortMenu();
	}
			...
			//第2层菜单
		      ...
		      if(IsSave()){
				*fItemSaved=TRUE;		//存储发生后，设备应复位
			    lcd_clear();
			    if(WritePortSetup(iPort,iItem,ports,MODE_NORMAL)==OK){	//save setup value
...
BYTE ReadPortSetup(BYTE iPort,BYTE iItem,BYTE *numItem,BYTE *des)
{
	...
	//sensor-ex
	  ...
	  Setup_SensorEx(iPort,iItem,numItem,des,MODE_NORMAL);
	  return OK;
	}
}
...
UINT sum;
	...
	FillBufWith(&inst[0],des_limit,0x88);

	inst[FRM_VER]=INST_HEADER;
	inst[FRM_LEN]=lenHEAD-1;
	inst[FRM_LEN+1]=0;
//	inst[FRM_DES_ADDR]=0xFF;
//	inst[FRM_DES_ADDR+1]=0xFF;
//	inst[FRM_DES_ADDR+2]=0xFF;
//	inst[FRM_DES_ADDR+3]=0xFF;
	inst[FRM_OP]=OP_READ_PORT_CFG;

	i=0;
	setbit0(i);				//bitINST_DIR		//inst
//	clrbit(i,bitNO_ANSWER_NEEDED);		//answer needed
//	setbit(i,bitIGNORE_TIME);			//no time stamp
	inst[FRM_PROPERTY_L]=i;
	inst[FRM_PROPERTY_H]=0;
...
BYTE *pTmp,*p;
BYTE task_index,tel_len,r,com_type;

	screen[FRM_VER]=INST_HEADER;
	screen[FRM_LEN]=lenHEAD-1;
	screen[FRM_LEN+1]=lenHEAD-1;
//	screen[FRM_DES_ADDR]=LoOfInt(ADDR_CARELESS);
//	screen[FRM_DES_ADDR+1]=HiOfInt(ADDR_CARELESS);
//	screen[FRM_DES_ADDR+2]=LoOfInt(ADDR_CARELESS);
//	screen[FRM_DES_ADDR+3]=HiOfInt(ADDR_CARELESS);
	screen[FRM_OP]=OP_WRITE_PORT_CFG;

	i=0;
	setbit0(i);		//,bitINST_DIR);			//inst
//	clrbit(i,bitNO_ANSWER_NEEDED);	//answer needed
//	setbit(i,bitIGNORE_TIME);		//no time stamp
	screen[FRM_PROPERTY_L]=i;
	screen[FRM_PROPERTY_H]=0;

/*	screen[PARA1]=iItem;						//index of item
	screen[PARA2]=numPORT0_SETUP_ITEMS;			//number of setup items
	screen[PARA3]=0;
	screen[PARA4]=0;
	screen[PARA5]=0;
	screen[PARA6]=0;  */
	sum=GetCRC(&screen[0],lenHEAD-2);
	screen[HCRC_L]=LoOfInt(sum);
	screen[HCRC_H]=HiOfInt(sum);
...
{
	CurX=0; CurY=0;
	lcd_cursor(0,L0); lcd_printf(&screen[DATA0]);
...
//外部传感器设置
//	FillBufWith(&msg[0],des_limit,0x88);

	msg[FRM_VER]=INST_HEADER;
//	msg[LEN_OF_INST]=lenHEAD-1;
//	msg[FRM_DES_ADDR]=LoOfInt(ADDR_CARELESS);
//	msg[FRM_DES_ADDR+1]=HiOfInt(ADDR_CARELESS);
//	msg[FRM_DES_ADDR+2]=LoOfInt(ADDR_CARELESS);
//	msg[FRM_DES_ADDR+3]=HiOfInt(ADDR_CARELESS);

	msg[FRM_OP]=OP_WRITE_PORT_CFG;
	msg[FRM_PROPERTY_L]=0;
	msg[FRM_PROPERTY_H]=0;
/*	msg[PARA1]=iItem;		//Port;
	msg[PARA2]=iPort;		//Ports;
	msg[PARA3]=0;
	msg[PARA4]=0;
	msg[PARA5]=0;
	msg[PARA6]=0;	*/

	msg[FRM_LEN]=LenToCursor();
	msg[FRM_LEN+1]=0;

	sum=GetCRC(&msg[0],lenHEAD-2);
	msg[HCRC_L]=LoOfInt(sum);
	msg[HCRC_H]=HiOfInt(sum);

	sum=GetCRC(&msg[DATA0],(int)(msg[FRM_LEN]-lenHEAD-1));
	*p++=LoOfInt(sum);
	*p=HiOfInt(sum);
//
	ConnectCell(iPort);
	Delay_ms(10);
//
	i=RWSerialSensor(&msg[0],msg[FRM_LEN]+1,&msg[0],des_limit,speed_mode);
	if(i==OK){
	  return OK;
	}
	else return FAILED;
...
//测站ID，Node address
	  case 1: 
		...
		SaveNodeAddress(p);
		//保存ID后，清除“停振计数”
//		ClrByte(M_CFG,MAP_XSTP_CNT);	//停振次数清零
...
//		r=GetComByte(tel_index);		
	  	com_type=(r>>3)&0x0f;
	    if((com_type==CONNECT_SM)||(com_type==CONNECT_BEIDOU)){
	      PointTo(p,SIGN_DEFAULT);	p++;	//while(*p!=SIGN_DEFAULT){p++;}; //point to 1st digit 
...
//
	iAddr=atoi((const char *)&d[0]);
  	mem_WriteData(M_CFG,(BYTE *)&iAddr,REG0+(UINT)CM_STA_ID,2);	
	return OK;
}
...
// 	mem_ReadData(M_CFG,REG0+(UINT)MAIN_UDISK_DAYS,&days,1);	
//
	days=GetByte(M_CFG,REG0+(UINT)MAIN_UDISK_DAYS);
...
//
	...
	WriteCfg(days,REG0+(UINT)MAIN_UDISK_DAYS);
	return OK;
}
*/
